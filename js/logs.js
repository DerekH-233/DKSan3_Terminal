...
export function joyImageDataUri() {
...
function buildItem(log, idx, today) {
    ...
    /* 点击 → 通用观测视图（二级页面，阅读器通用化）；特别记录带 special 参数 */
    const open = () => {
        const q = `date=${encodeURIComponent(log.date || '')}${isSpecial ? '&special=1' : ''}`;
        location.href = `view.html?${q}`;
    };
